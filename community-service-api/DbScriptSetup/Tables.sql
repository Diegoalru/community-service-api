...
COMMENT ON COLUMN TIPO_CORRESPONDENCIA.DESCRIPCION IS 'Descripción del tipo de corresponsalidad (e.g., Email, Teléfono, Celular)';
...
(
  ...
  CONSTRAINT FK_CORRESPONDENCIA_USUARIO FOREIGN KEY (ID_USUARIO) REFERENCES USUARIO (ID_USUARIO),
...
COMMENT ON TABLE CORRESPONDENCIA IS 'Tabla de correspondencia de datos personales';
COMMENT ON COLUMN CORRESPONDENCIA.ID_USUARIO IS 'Identificador del usuario';
...
(
  ...
  CONSTRAINT CK_ORG_EMAIL CHECK (EMAIL LIKE '%@%.%'),
  CONSTRAINT CK_ORG_SITE CHECK (SITIO_WEB LIKE '%://%'),
...
COMMENT ON COLUMN ORGANIZACION.EMAIL IS 'Correo electrónico de la organización';
COMMENT ON COLUMN ORGANIZACION.TELEFONO IS 'Número de teléfono de la organización';
COMMENT ON COLUMN ORGANIZACION.SITIO_WEB IS 'Sitio web de la organización';
...
COMMENT ON COLUMN ROL_USUARIO_ORGANIZACION.ESTADO IS 'Estado del registro: Activo (A) o Inactivo (I)';
...
(
  ...
  CONSTRAINT CK_CERT_PART_SIT CHECK (SITUACION IN ('E', 'P', 'A')),
...
COMMENT ON COLUMN CERTIFICADO_PARTICIPACION.SITUACION IS 'Situación del certificado: Pendiente (P), Emitido (E), Anulado (A)';
